...
def create_view():
    global conn, c
    c.execute('''CREATE VIEW OrderSize(oid, size) AS
...
def drop_view():
    global conn, c
    c.execute('DROP VIEW IF EXISTS OrderSize;')
    conn.commit()
    return
...
def Q2():
    ...
    cpc = c.fetchone()
    c.execute('SELECT AVG(size) FROM OrderSize;')
    avg = c.fetchone()[0]
    conn.commit()
    ...
    rows1 = c.fetchall()
    if len(rows1)>1:
        rows = [item for t in rows1 for item in t]
    else:
        rows = rows1
    a = 0
    total = 0
    for i in rows:
        oid = rows[a]
        oid = str(oid)
        c.execute('''SELECT COUNT(DISTINCT oid)
                     FROM OrderSize, Orders
                     WHERE oid = order_id
                     AND size>? AND order_id=?;''', (avg, oid))
        count = c.fetchone()
        a = a + 1
        total = total + count[0]
    conn.commit()
