...
def user_optimized():
    global conn, c
    c.execute('PRAGMA foreign_keys=ON;')
    c.execute('CREATE INDEX sellerid ON Sellers(seller_id);')
...
def Q4():
    global conn, c
    c.execute('SELECT customer_id FROM Orders GROUP BY customer_id HAVING COUNT(*) > 1 ORDER BY random() LIMIT 1;')
    rows = c.fetchall()
    a = 0
    total = 0
    for i in rows:
        cid = rows[a]
        c.execute('''SELECT COUNT(DISTINCT Sellers.seller_postal_code)
                     FROM Order_items
